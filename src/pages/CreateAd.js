...
class CreateAd extends React.Component {
  ...
  getProducts = () => {
    ...
    .get(
      ...
      {
        headers: {
          Authorization: "joao-vitor-alves",
        },
      }
    ).then((res) => {
  ...
  createProduct = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        this.setState({ inputPaymentMethod: "" });
        this.setState({ inputCategory: "" });
        this.setState({ inputPhotos: "" });
        this.setState({ inputInstallments: "" });
  ...
  deleteProduct = (productId) => {
    axios.delete(`https://us-central1-labenu-apis.cloudfunctions.net/eloFourTwo/products/${productId}`, 
    {
        headers: {
            Authorization: "joao-vitor-alves"
        }
  ...
  render(){
    const listOfProducts = this.state.products.map((product) => (
        <div>
            ...
            <button onClick={() => this.deleteProduct(product.id)}>Apagar produto</button>
        </div>
    ))
    ...
    return (<DivContainer>
        ...
        <Input value={this.state.inputName} onChange={this.handleName}> </Input>
        <label>Descrição do produto</label>
        <Input value={this.state.inputDescription} onChange={this.handleDescription}> </Input>
        <label>Preço: </label>
        <Input value={this.state.inputPrice} onChange={this.handlePrice}> </Input>
        <label>Métodos de pagamento: </label>
        <Input value={this.state.inputPaymentMethod} onChange={this.handlePaymentMethod}> </Input>
        <label>Categoria: </label>
        <Input value={this.state.inputCategory} onChange={this.handleCategory}> </Input>
        <label>Fotos: </label>
        <Input value={this.state.inputPhotos} onChange={this.handlePhotos}> </Input>
        <label>Parcelas: </label>
        <Input value={this.state.inputInstallments} onChange={this.handleInstallments}> </Input>
        </DivInputs>
        <DivProducts>
        <h1>Meus produtos</h1>
        {listOfProducts}
